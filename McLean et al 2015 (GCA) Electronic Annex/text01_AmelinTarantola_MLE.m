%% first using 981 + 202-205 data,
    % assume 
         %1. 'minor' IC of tracer, and IC of Pb blank (pers comm Amelin)
         %2. IC of BaPO2 and Tl
         %3. 208/206 of 981 (Catanzaro)
         %4. measured data of Amelin and Davis
    %to determine 
        %1. the 202/205 of the (Amelin) tracer
        %2. the the BaPO2 interference magnitude
        %3. the gravimetric/tracer ratio
        %4. the (inter-calibrated) 204/206 and 207/206 of 981, 982, and Puratronic Pb
...
r86g_981 = 2.1681; sr86g_981 = 0.0004;
...
start1 = 1; start2 = size(r52BaPbs_981,2)+1; start3 = size(r52BaPbs_981,2)+size(r52BaPbs_982,2)+1; %new code since no padarray
...
r52BaPbs = zeros(maxblocks, size(betas_981,2)+size(betas_982,2)+size(betas_Pur,2)); %replacing padarray code
r52BaPbs(1:size(r52BaPbs_981,1),start1:start2-1) = r52BaPbs_981;
r52BaPbs(1:size(r52BaPbs_982,1),start2:start3-1) = r52BaPbs_982;
r52BaPbs(1:size(r52BaPbs_Pur,1),start3:end)      = r52BaPbs_Pur;
...
betas = zeros(maxblocks, size(betas_981,2)+size(betas_982,2)+size(betas_Pur,2)); %replacing padarray code
betas(1:size(betas_981,1),start1:start2-1) = betas_981;
betas(1:size(betas_982,1),start2:start3-1) = betas_982;
betas(1:size(betas_Pur,1),start3:end)      = betas_Pur;
